...
vim.api.nvim_create_autocmd("LspAttach", {
	...
	callback = function()
		...
		map("gt", fzf_lua.lsp_typedefs, "[G]oto [T]ypedef")
...
map("<leader>st", fzf_lua.treesitter, "[S]earch [T]reesitter symbols in current buffer")

-- Manpages
map("<leader>smp", fzf_lua.manpages, "[M]an [P]ages")

-- Colorschemes
local colorscheme_persistence = require("config.colorscheme-persistence")
map("<leader>scs", colorscheme_persistence.select_colorscheme, "[C]olor[S]chemes")
